...
#define NumDirEntries			64 //64 file/subdirectories per directory
#define DirectoryFileSize		sizeof(DirectoryEntry)*64
#define MaxDepth			255
...
class Directory {
  public:
    ...
    void List();			// Print the names of all the files
					//  in the directory
	void RecList(int depth);
    void Print();			// Verbose print of the contents
					//  of the directory -- all the file
					//  names and their contents.
	bool AddDirectory(char *name, int Sector, PersistentBitmap *freeMap);
	...
	bool IsFile(char *name)
	{
		...
		if( i != -1)
			return table[i].isFile;
		return -1;
	}
 
  static int cutPath(char *name, char (&path)[MaxDepth][FileNameMaxLen+1])
  {
    int depth = 0,j = 0, i = 1;
  	path[depth][j] = name[0]; // read the first "/"
  	j++;
  	while(name[i]!='\0') 
  	{
  		if(name[i] == '/')
  		{
  			if(depth >= MaxDepth)
  			{
  				printf("Error : the path length is large than 255\n");
  				break;
  			}		
  			else
  			{
