...
// Create a Nachos file of the same length
    ...
    if (!kernel->fileSystem->Create(to, fileLength)) {   // Create Nachos file
        ...
        Close(fd);
		//printf("End of Main.cc Copy1\n");
        return;
    }
    //printf("End of Main.cc Copy2\n");
    openFile = kernel->fileSystem->Open(to);
    ASSERT(openFile != NULL);
    //printf("End of Main.cc Copy3\n");
...
{
	//printf("Call CreateDirectory\n");
	// MP4 Assignment
	kernel ->fileSystem ->CreatDir(name);
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
		...
		mkdirFlag = true;
		i++;
		//printf("Main.cc CreatDir name %s\n",createDirectoryName);
	}
